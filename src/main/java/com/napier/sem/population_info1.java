...
public class population_info1 {
    ...
    public void getCities_population_con1(Connection con1)
    {
        try
        {
            ...
            String strSelect =
                    "SELECT country.Continent, SUM(country.Population) AS con_population, SUM(city.Population) AS city_population FROM city INNER JOIN country ON city.CountryCode = country.Code GROUP BY Continent";
            // Execute SQL statement
            ResultSet rset = stmt.executeQuery(strSelect);
            // Print header
            System.out.println("----------------------------------------------------------------------------------------------------------------------------------------------------");

            System.out.format("%20s %30s %30s %30s %33s",
                        "Continent", "Population of cities", "Population % of cities", "Population outside of cities", "Population % outside of cities");
            System.out.println();
            System.out.println("----------------------------------------------------------------------------------------------------------------------------------------------------");
...
//                System.out.println("Population Percentage of people who live in cities: "+not_cities_population_per +"%");
                System.out.format("%20s %30d %30f %30d %33f",
                        continent, cities_population, cities_population_per, population_not, not_cities_population_per);
